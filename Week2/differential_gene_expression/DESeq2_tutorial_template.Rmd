...
dds_filterd <- dds[rowSums(assays(dds)$counts) > 0, ]
head(dds_filterd)
...
rlog_transformed_dds <- rlog(dds_filterd, blind = TRUE)
...
vst_transformed_data <- vst(dds_filterd, blind = T)
...
vst_cor_matrix <- cor(assay(vst_transformed_data), method = "spearman")

# use pheatmap to plot the correlation matrix 
pheatmap(cor_matrix)
pheatmap(vst_cor_matrix)
...
plotPCA(rlog_transformed_dds_subset, intgroup="genotype") + theme(aspect.ratio=1)

# intgroup replicate
plotPCA(rlog_transformed_dds_subset, intgroup="replicate") + theme(aspect.ratio=1)
...
dds_filtered_updated <- dds_filterd[,1:6]
...
#The values are almost the same showing there is little differences in library size
...
ggplot(size_factors_df, aes(rownames(size_factors_df),y)) +
  geom_col()
...
dds_filtered_final <- dds_filterd[,1:6]
...
plotCounts(dds_filtered_final,
           gene = "ENSMUSG00000025900",
           intgroup = "cell_type",
           returnData = T) # returnData=T will give us the data only for the gene and can be used to plot graphs with it
```
```{r}
returned_data <- plotCounts(dds_filtered_final,
           gene = "ENSMUSG00000025900",
           intgroup = "cell_type",
           returnData = T)

ggplot(returned_data, aes(x=rownames(returned_data), y=count)) +
  geom_col(aes(color=cell_type, fill=cell_type))
```
...
results_CD4_CD8 <- results(dds_final, alpha = 0.05, contrast)
print(summary(results_obj), summary(results_CD4_CD8))
```
...
```{r}
results_CD4_CD8_df <- data.frame(
  results_CD4_CD8
)
ggplot(results_obj, aes())
...
shrnkenLFC_dds_results_df <- shrnkenLFC_dds_results_df %>% 
						rownames_to_column("ensembl_gene_ids")

# Save results to file 
write_csv(shrnkenLFC_dds_results_df, "CD8_vs_CD4_DE_results_lfcshrinkage_allgenes.csv")
```
