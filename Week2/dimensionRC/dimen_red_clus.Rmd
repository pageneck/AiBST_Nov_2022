...
```{r}
pca_iris_species <- ggplot(pca_iris_dataframe, aes(x=PC1, PC2)) + 
...
```{r}
variance_dataframe <- data.frame( explained_variance  =explained_variance_ratio,
                                  principle_component= c("PC1","PC2","PC3","PC4"))
  
head(variance_dataframe)
```

```{r}
ggplot(variance_dataframe, aes(x=principle_component, y=explained_variance)) +
  geom_col()
...
```{r}
ggplot(umap_iris_dataframe, aes(x=V1,V2)) + 
  geom_point()
...
iris_hclust_average_labels <- cutree(hclust_iris_average, k=4)
...
iris_hclust_single_labels <- cutree(hclust_iris_single, k=4)
...
plot_average <- ggplot(iris_clusters_dataframe, aes(x=Sepal.Length,y=Sepal.Width )) + geom_point(aes(color=hclust_average))
  
  
plot_complete <- ggplot(iris_clusters_dataframe,aes(x=Sepal.Length,y=Sepal.Width )) + geom_point(aes(color=hclust_complete))
  
  
plot_single <- ggplot(iris_clusters_dataframe, aes(x=Sepal.Length,y=Sepal.Width )) + geom_point(aes(color=hclust_single))
  
  
plot_ward <- ggplot(iris_clusters_dataframe,aes(x=Sepal.Length,y=Sepal.Width )) + geom_point(aes(color=hclust_ward))
...
```{r}
dbscan_plot <- ggplot(iris_clusters_dataframe,aes(Sepal.Length, Sepal.Width)) +
  geom_point(aes(color=dbscan))
...
```{r}
hdbscan_plot <- ggplot(iris_clusters_dataframe,aes(Sepal.Length,Sepal.Width)    ) +
  geom_point(aes(color=hdbscan))
...
```{r}
cluster_coord <- kmeans_iris$centers
cluster_coord
```
...
```{r}
ggplot(iris_labelled, aes(Sepal.Length, Sepal.Width)) +
...
```{r}
iris_means_centers <- as.data.frame(cluster_coord)
...
geom_point() +
geom_point(data=iris_means_centers, shape = "*", size = 10)
