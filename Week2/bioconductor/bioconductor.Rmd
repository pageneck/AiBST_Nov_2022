...
```{r}
Ebox <- "CANNTG"
countPattern(Ebox, subject = chrY, fixed = F)
```
```{r}
countPattern(pattern = Ebox, subject = DNAString(letter(chrY, 1E6:2E6)), fixed = FALSE)
```
...
```{r}
#get transcript identifier for each range
gtf_ACTB$transcript_id
length(gtf_ACTB$transcript_id)
#remove the NAs
na.omit(gtf_ACTB$transcript_id)
length( na.omit(gtf_ACTB$transcript_id))

#Remove the duplicates
unique(na.omit(gtf_ACTB$transcript_id))
unique(length(na.omit(gtf_ACTB$transcript_id)))
...
```
```{r}
length(gtf_ACTB_transcripts)
```
```{r}
gtf_ACTB_transcripts$ENST00000417101
```
```{r}
reduce(gtf_ACTB_transcripts$ENST00000417101)
```
```{r}
ranges(gtf_ACTB_transcripts$ENST00000417101)
```
...
```{r}
AnnotationDbi::select(x=org.Hs.eg.db, keys = human_symbols,
                      columns = c("ENTREZID","ENSEMBL","OMIM"),
                      keytype = "SYMBOL")
```
...
```{r}
demo_se <- SummarizedExperiment(
  assays = SimpleList(counts=assay_counts),
