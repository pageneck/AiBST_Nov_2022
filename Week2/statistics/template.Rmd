...
```
```{r}
quantile(x1, seq(0,1,length.out=11))
...
abline(v=median(x1), col="red",lty = "dashed")
abline(v=(mean(x1)-2*sd(x1)), col="blue")
abline(v=(mean(x1)+2*sd(x1)), col="blue")
abline(v=median(x1)- mad(x1), col="blue", lty="dashed")
abline(v=median(x1)+ mad(x1), col="blue", lty="dashed")
...
```{r}
x3 <- pnorm(q=seq(-5,5,by=0.1))
plot(seq(-5,5,by=0.1), x3)
...
plot(seq(0,1,0.01),qnorm(p=seq(0,1,0.01)))
abline(h=0, col="blue")
...
```{r}
t.test(gene_exprs ~ group, data = df)
```

**What is the p-value?**
```{r}
# This will break the code
# t.test(df$gene_exprs, df$group)
# But this might work
t.test2 <- t.test(df$gene_exprs ~ df$group)
str(t.test2)
```
...
```
```{r}
is.matrix(logcounts_matrix)
```
```{r}
print(logcounts_matrix)
```
```{r}
...
```{r}
which.min(result_bh)
row.names(logcounts_matrix)[which.min(result_bh)]
gene_table$gene_name[gene_table$gene_id==row.names(logcounts_matrix)[which.min(result_bh)]]
